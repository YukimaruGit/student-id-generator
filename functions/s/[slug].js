...
export default async function onRequest({ request }) {
  ...
  // [slug]は1つのセグメントとして受け取るが、実際のパスは複数セグメント
  const pathSegments = url.pathname.split('/').filter(Boolean);
  
  // /s/v123/a/b/c の場合、pathSegments = ['s', 'v123', 'a', 'b', 'c']
  if (pathSegments.length < 3 || pathSegments[0] !== 's') {
    return new Response("Not found", { status: 404 });
  }
  
  const versionSeg = pathSegments[1];
  const version = versionSeg.startsWith('v') ? versionSeg.slice(1) : '';
  const publicId = pathSegments.slice(2).join('/'); // 残りのセグメントを結合
